...
| OBS-002 | Privacy | 监控事件不含简历/JD/答案/报告/OTP/token/密钥全文 | `evidence/IB-06-admin-observability.md` + `evidence/IB-09-tencent-rum-frontend.md`；首条远端错误样本脱敏复核通过；3005 endpoint 均正确但本地 `beforeRequest` 误拦截 `whiteList/null` 控制面 envelope，现仅对该无 payload 形态作严格放行，待下一候选复核真实请求字段 | Partial |
| OBS-003 | Traceability | API response、结构化日志和监控可用 requestId 关联 | `evidence/IB-06-admin-observability.md` + `evidence/IB-09-tencent-rum-frontend.md`；3005 无痕页确认 adapter/Aegis/最终 endpoint 正常但控制面和 receiver 仍为零；严格控制面放行修正待下一候选完成 Session 401 的 response/RUM/local log 同 requestId 对账 | Partial |
