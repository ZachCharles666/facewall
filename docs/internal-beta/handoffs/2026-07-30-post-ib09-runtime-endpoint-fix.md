...
- Public Nginx upstream and `passbuddy-local-readiness` target are on port 3003.
- Release: `/home/ubuntu/releases/passbuddy-20260729-caa52ee`
- PM2 process: `facewall-rum-fix-candidate`
...
- Rollback files:
  - `/etc/nginx/conf.d/facewall.conf.pre-ib09fix-20260729-042719`
  - `/etc/passbuddy/readiness.env.pre-ib09fix-20260729-042719`
- Ports 3000, 3001, 3002, and 3003 were all listening at last check. No `pm2 save`, old-process cleanup, OTP, alert notification, or real pilot was performed.
...
- Current uncommitted adapter fix:
  - initialize with mainland `hostUrl`;
  - immediately call the public instance `setConfig`;
  - retain log/PV/speed/performance/web-vitals and `/rateConfig`;
  - keep whitelist/custom event/custom time/offline endpoints empty.
...
- Source security: Pass, 196 files.
...
1. Review the six-file runtime-fix scope and rerun only checks needed to detect drift.
...
6. Preserve the current 3003 release and timestamped Nginx/readiness rollback configs before any cutover.
7. On the next public candidate, open a fresh page with Network recording first:
   - expect mainland `/rateConfig` and receiver traffic;
   - require no whitelist request;
